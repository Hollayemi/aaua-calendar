...
const filtered = cate !== "All" ?  events.filter((each) => each.category === cate) : events
    ...
    return (
        <AppWrapper
            pageName={!isback ? "Event" : "Event Description"}
            back={() => setBack(false)}
        >
            <div className="flex">
                <div
                ...
                >
                    <div className="py-6 flex items-center gbbdy overflow-auto overflowStyle sticky top-0  z-40">
                        {ava.map((each, i) => (
                            <ItemsTab
                                name={each}
                                key={i}
                                category={cate}
                                onClick={() => setCate(each)}
                            />
                        ))}
                    </div>
                    <br />
